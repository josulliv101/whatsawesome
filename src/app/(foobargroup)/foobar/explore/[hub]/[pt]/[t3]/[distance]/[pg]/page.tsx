...
export default async function Page({
  ...
  const startAt = page * hitsPerPage + 1;
  const endAt = page * hitsPerPage + hitsPerPage;
  const totalPages = Math.ceil(nbHits / hitsPerPage);

  const resultsText = `Showing results ${startAt}-
${Math.min(endAt, nbHits)} of ${nbHits}`;
  return (
    <div>
      <Breadcrumb>
        <div className="capitalize text-xs  flex items-center gap-4">
          ...
          {pt !== "index" && t3 !== "index" ? (
            <div className="flex items-center gap-2">
              <span>{pt}</span>
              <BadgeCheckIcon
                className={`h-4 w-4 mr-0 ml-2 text-white opacity-80`}
              />
              <span>{t3}</span>
            </div>
          ) : (
            ...
            <div className="rounded-md bg-muted px-4 py-0 pb-6 flex flex-col gap-4">
              {hits?.map(
                (
                  ...
                  const cacheTag = getCacheTagFromParams(
                    hub,
                    [pt, t3],
                    distance
                  ); // [hub, "place", pt, t3, distance].join("-");
